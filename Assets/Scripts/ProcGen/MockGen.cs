...
namespace ProcGen
{
  ...
  public class Generator
  {
    ...
    public void Generate(Config config)
    {
      ...
      else
      {
        ...
        SpawnProp(finish, _prefabs[(int)Prefab.Terminal], 4, 0);
        SpawnProp(finish, _prefabs[(int)Prefab.Terminal], 3, 0);
        SpawnProp(finish, _prefabs[(int)Prefab.Terminal], 1, 0);
        SpawnProp(finish, _prefabs[(int)Prefab.Terminal], 0, 0);
      ...
      var surface = _config.GO.AddComponent<Unity.AI.Navigation.NavMeshSurface>();
    ...
    void GenProps(Room room)
    {
      ...
      int propAmt = 10;
      var iterations = groundAmt / propAmt * propFrequency;
    ...
    {
      for (int i = 0; i < _random.Next(InnerMin, InnerMax); ++i)
      {
        room.ComputeEmpty();
    ...
    void SpawnProp(Room room, GameObject prefab, int groundIndex, float angMax = 360)
    {
      var prop = _config.Instantiate(prefab, (Vector3)room.TakeGround(groundIndex) * BlockSize + room.Pos,
        Quaternion.AngleAxis((float)_random.NextDouble() * angMax, Vector3.up));
    var networkObject = prop.GetComponentInChildren<Unity.Netcode.NetworkObject>();
  ...
  public class MockGen : MonoBehaviour
  {
    public List<PremadeRoom> PremadeRooms = new List<PremadeRoom>();
    public void Start()
    {
