...
## Procedural Generation - Jared Givens

The Procedural generation creates the map based on a  [config](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L348) that takes the seed, instantiate function, and parent object. After creating a config the user can easily generate a map similar to the test class [MockGen](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L776). The generator creates a list of rooms that it links together. The generator [creates doors](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L605) in the room based on the size of the room and the size of the previous room, given we are going in a direction tangent to the total direction. The generator chooses a direction to be the total direction from 4 cardinal directions. The total direction dictates the first and overall direction of the map. Each room creates a matrix of enum values corresponding to prefabs to be [instantiated](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L542) in their place. The Generator spawns props by counting the ground locations and choosing a random index to spawn at then in [TakeGround](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L107) room then looks for that location and marks the ground to keep multiple from spawning stacked.

After a room is created, there is a random chance to [change the generation direction](https://github.com/abhiss/symmetrical-octo-waffle/blob/79d8a0c3317ea0efc8d1080daf2df19d487f63b1/Assets/Scripts/ProcGen/MockGen.cs#L748) from total to an adjacent direction or from an adjacent direction back to total. The System also supports premade rooms by making a room according to the grid dimesions and adding it to the generators list with an index. The index indicates where in the list it should be substutuded for a procedural room.
